...
import { Calendar, Heart, X, Loader } from 'lucide-react'
import axios from 'axios'

function DailyChallengeModal({ isOpen, onClose, onComplete }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        onComplete(response.data.esCorrecta)
      }, 2000)
    } catch (err) {
  ...
  return (
    <div className="modal-overlay">
      <div className="card-pixel max-w-3xl w-full max-h-[90vh] overflow-y-auto relative">
        {/* Botón cerrar */}
        <button
          onClick={handleSkip}
        ...
        <div className="text-center mb-6">
          <Calendar className="w-16 h-16 text-blue-400 mx-auto mb-4 glow-animation" strokeWidth={2.5} />
          <h2 className="text-pixel text-3xl text-white mb-2">
            DESAFÍO DIARIO
          </h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-game text-lg text-gray-400 mb-6">
              Vuelve mañana para un nuevo desafío
            </p>
            <button onClick={handleSkip} className="btn-pixel">
              CONTINUAR AL JUEGO
            </button>
          </div>
        ) : resultado ? (
          <div className="text-center py-12">
            {resultado.esCorrecta ? (
              <>
                ...
                <h3 className="text-pixel text-3xl text-green-400 mb-4">
                  ¡CORRECTO!
                </h3>
              ...
              <>
                ...
                <h3 className="text-pixel text-3xl text-red-400 mb-4">
                  INCORRECTO
                </h3>
